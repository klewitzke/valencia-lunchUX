...
if(isset($_GET['editId'])) {
	...
	$cat8 = 0;
	$cat9 = 0;
	$_SESSION['AdultNumber'] = $_GET['editId'];
	$guid = $_SESSION['adults'][$_SESSION['AdultNumber']][0];
	foreach($_SESSION['Incomes'] as $key_1=>$value_1) {
		if($value_1[0]==$guid) {
			if($value_1[1]==5) $cat5 = 1;
			if($value_1[1]==6) $cat6 = 1;
			if($value_1[1]==7) $cat7 = 1;
			if($value_1[1]==8) $cat8 = 1;
			if($value_1[1]==9) $cat9 = 1;
		}
	}
} else {
	$_SESSION['AdultNumber'] = 0;
	do {
		if($_SESSION['AdultNumber']==(count($_SESSION['adults']))){
...
<script>
	$(function() {
		$(document).on("click", ".option-full-text,#opt1-close,#opt2-close,#opt3-close,#opt4-close,#opt5-close", function(event) {
			$( "#next" ).show();
			...
			} else if (($(this).attr("id") == "opt5-text")||($(this).attr("id") == "opt5-close")) {
				$("#opt5").toggleClass("selected");
			} else if ($(this).attr("id") == "opt6-text") {
			...
			if($("#opt5").hasClass("selected")) {
				$("#opt5-table,#opt5-add,#opt5-close").show();
				$(".opt5-wage,.opt5-frequency").prop('required',true);
				$(".opt5-wage,.opt5-frequency").prop('disabled',false);
			} else {
				$("#opt5-table,#opt5-add,#opt5-close").hide();
				$(".opt5-wage,.opt5-frequency").removeAttr('required');
				$(".opt5-wage,.opt5-frequency").prop('disabled','disabled');
			}
		...
		$(document).on("click", "#opt5-add", function(event){
			var newRow = jQuery('<tr><td style="color:white;">$</td><td style="width:50%"><input class="opt5-wage" type="tel" name="opt5-wageamt[]" style="text-align:center;margin:0;" placeholder="Amount" autocomplete="off" required /></td><td style="width:50%"><select name="opt5-frequency[]" class="opt5-frequency"  required ><option disabled selected>Frequency</option><option value="1">Weekly</option><option value="2">Every 2 Weeks</option><option value="3">Twice Monthly</option><option value="4">Monthly</option></select></td><td id="remove" style="color:white;font-size:16px;">Remove</td></tr>');
			$("#opt5-table").append(newRow);
		});
		...
		$(document).on("blur", ".opt1-wage,.opt2-wage,.opt3-wage,.opt4-wage,.opt5-wage", function(event) {
...
<div id="main">
<form action="hh_member_income_process.php" method="post" id="form">
...
<?php
echo(strtoupper($_SESSION['adults'][$_SESSION['AdultNumber']][1]));
?>
...
// Military pay
echo('<div  id="opt2" class="option-full');
if(isset($_GET['editId']) && $cat6==1) echo(' selected');
echo('" onClick="return false;"><div class="option-full-text" id="opt2-text">Receives military pay</div><br />');
if(isset($_GET['editId']) && $cat6==1) {
...
if(isset($_GET['editId']) && $cat7==1) echo(' selected');
echo('" onClick="return false;"><div class="option-full-text" id="opt3-text">Receives income from public assistance, alimony or child support</div><br />');
if(isset($_GET['editId']) && $cat7==1) {
...
echo('<div  id="opt4" class="option-full');
if(isset($_GET['editId']) && $cat8==1) echo(' selected');
echo('" onClick="return false;"><div class="option-full-text" id="opt4-text">Receives retirement or disability income</div><br />');
if(isset($_GET['editId']) && $cat8==1) {
...
echo('<div  id="opt5" class="option-full');
if(isset($_GET['editId']) && $cat9==1) echo(' selected');
echo('" onClick="return false;"><div class="option-full-text" id="opt5-text">Receives investment income or income from any other source</div><br />');
if(isset($_GET['editId']) && $cat9==1) {
	echo('<table id="opt5-table">');
	$i=0;
	foreach($_SESSION['Incomes'] as $key_1=>$value_1) {
		if($value_1[0]==$guid) {
			if($value_1[1]==9) {
				echo('<tr><td style="color:white;">$</td><td style="width:50%"><input class="opt5-wage" type="tel" name="opt5-wageamt[]" value="'.$value_1[2].'" style="text-align:center;margin:0;" placeholder="Amount" autocomplete="off" /></td><td style="width:50%"><select name="opt5-frequency[]" class="opt5-frequency"  ><option disabled>Frequency</option><option value="1"');
				if($value_1[3]==1) echo(' selected');
				echo('>Weekly</option><option value="2"');
				if($value_1[3]==2) echo(' selected');
				echo('>Every 2 Weeks</option><option value="3"');
	...
	echo('</table><button id="opt5-add" class="cancel" style="width:45%;" ><i class="fa fa-usd"></i> Add</button><button id="opt5-close" class="cancel" style="width:45%;"><i class="fa fa-times"></i> Cancel</button></div>');
} else {
	echo('<table id="opt5-table" style="display:none;"><tr><td style="color:white;">$</td><td style="width:50%"><input class="opt5-wage" type="tel" name="opt5-wageamt[]" style="text-align:center;margin:0;" placeholder="Amount" autocomplete="off" disabled /></td><td style="width:50%"><select name="opt5-frequency[]" class="opt5-frequency"  disabled ><option disabled selected>Frequency</option><option value="1">Weekly</option><option value="2">Every 2 Weeks</option><option value="3">Twice Monthly</option><option value="4">Monthly</option></select></td><td id="remove" style="color:white;font-size:16px;visibility:hidden;">Remove</td></tr></table><button id="opt5-add" class="cancel" style="width:45%;display:none;" ><i class="fa fa-usd"></i> Add</button><button id="opt5-close" class="cancel" style="width:45%;display:none;"><i class="fa fa-times"></i> Cancel</button></div>');
}

?>
<!-- None of the above -->
<div  id="opt6" class="option-full
<?php if(isset($_GET['editId']) && $cat5==0 && $cat6==0 && $cat7==0 && $cat8==0 && $cat9==0) echo(' selected'); ?>
" onClick="return false;"><div class="option-full-text" id="opt6-text">None of the above</div></div>
